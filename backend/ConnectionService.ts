...
export interface Friend {
  ...
  unreadCount: number;
  isCloseFriend?: boolean; // "Melhores Amigos"
  isFollowing?: boolean; // Se eu sigo este usuário (usado na lista de seguidores)
}
...
class ConnectionService {
  ...
  // Verifica se eu sigo o usuário target
  ...
  // Quem segue este usuário (Aparece na aba "Seguidores")
  public async getFollowers(targetUserId?: string): Promise<Friend[]> {
      ...
      // Se estivermos vendo nosso próprio perfil, precisamos saber quem DESSES seguidores NÓS seguimos de volta
      ...
      return data.map((row: any) => ({
          ...
          status: 'offline',
          unreadCount: 0,
          isFollowing: myFollowingIds.has(row.profiles.id) // Importante para o botão "Seguir de Volta"
      }));
  }

  // Quem este usuário segue (Aparece na aba "Seguindo")
  public async getFollowing(targetUserId?: string): Promise<Friend[]> {
      ...
      // Verificar status de amizade para cada pessoa que sigo (para saber se mostro "Add Amigo")
      // Esta abordagem não é a mais performática para listas gigantes, mas funciona para o MVP.
      const enrichedList = await Promise.all(data.map(async (row: any) => {
          let fStatus: 'none' | 'pending_sent' | 'pending_received' | 'accepted' = 'none';
          
          if (currentUserId) {
             fStatus = await this.getFriendshipStatusWithUser(currentUserId, row.profiles.id);
          }
          ...
          return {
              ...
              name: row.profiles.username || 'Usuário',
              avatar: row.profiles.avatar_url || '',
              status: 'offline' as 'offline', // Type casting fix
              unreadCount: 0,
              isFollowing: true,
  ...
  // Helper interno para checar amizade sem depender do active user global
  private async getFriendshipStatusWithUser(myId: string, targetId: string): Promise<'none' | 'pending_sent' | 'pending_received' | 'accepted'> {
      const { data } = await supabase
          .from('friendships')
          .select('*')
  ...
  public async getFriendshipStatus(targetId: string): Promise<'none' | 'pending_sent' | 'pending_received' | 'accepted'> {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user || targetId === 'current_user') return 'none';
      return this.getFriendshipStatusWithUser(user.id, targetId);
  }

  // Lista de Amigos (Accepted)
  public async getFriends(targetUserId?: string): Promise<Friend[]> {
    ...
    return data.map((row: any) => {
        ...
        const isClose = isRequester ? row.is_close_friend_requester : row.is_close_friend_receiver;
  ...
  // Lista de Solicitações Pendentes (Recebidas)
  public async getPendingRequests(targetUserId?: string): Promise<Friend[]> {
      ...
      // Buscar onde eu sou o receiver e status é pending
      const { data, error } = await supabase
          .from('friendships')
          .select(`
              requester_id, 
  ...
  public async requestFriendship(targetId: string): Promise<{ success: boolean, message: string }> {
      const { data, error } = await supabase.rpc('request_friendship', { target_id: targetId });
  ...
  public async getConversations(): Promise<Friend[]> {
      ...
      // Query mockada para listar conversas baseada na tabela de mensagens (não otimizada)
      // Em produção, usar tabela de 'conversations' ou query distinta de latest message
      const { data } = await supabase
  ...
  public async sendMessage(friendId: string, content: string, type: 'text' | 'image' | 'video' | 'audio' | 'promotion_request' = 'text', mediaUrl?: string): Promise<PrivateMessage> {
      const { data: { user } } = await supabase.auth.getUser();
  ...
  public async toggleCloseFriend(friendId: string): Promise<boolean> {
      // Mock logic as it requires schema update for close friends column in friendships
      return true;
  }
}
