...
import { X, Zap, Bookmark, Send, Maximize2, ZoomIn, ZoomOut, RotateCcw, Plus, Image as ImageIcon, Upload, Loader2, Hash, Check, UserPlus, UserCheck, List, Camera, MessageCircle, Clock, CheckCircle, Footprints, User } from 'lucide-react';
...
const ZoomViewer: React.FC<{ imageUrl: string; onClose: () => void }> = ({ imageUrl, onClose }) => {
    // ... (ZoomViewer implementation same as before)
    ...
    const handleWheel = (e: React.WheelEvent) => { e.stopPropagation(); const delta = -e.deltaY * 0.001; setScale(s => Math.min(Math.max(0.5, s + delta), 5)); };
    const handleMouseDown = (e: React.MouseEvent) => { e.preventDefault(); setIsDragging(true); dragStart.current = { x: e.clientX, y: e.clientY }; lastPosition.current = { ...position }; };
    const handleMouseMove = (e: React.MouseEvent) => { if (!isDragging) return; setPosition({ x: lastPosition.current.x + (e.clientX - dragStart.current.x), y: lastPosition.current.y + (e.clientY - dragStart.current.y) }); };
    const handleMouseUp = () => setIsDragging(false);
    const handleTouchStart = (e: React.TouchEvent) => { if (e.touches.length === 1) { setIsDragging(true); dragStart.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }; lastPosition.current = { ...position }; } else if (e.touches.length === 2) { lastTouchDistance.current = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY); } };
    const handleTouchMove = (e: React.TouchEvent) => { if (e.touches.length === 1 && isDragging) { setPosition({ x: lastPosition.current.x + (e.touches[0].clientX - dragStart.current.x), y: lastPosition.current.y + (e.touches[0].clientY - dragStart.current.y) }); } else if (e.touches.length === 2 && lastTouchDistance.current !== null) { const dist = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY); const delta = dist - lastTouchDistance.current; setScale(s => Math.min(Math.max(0.5, s + delta * 0.005), 5)); lastTouchDistance.current = dist; } };
    const handleTouchEnd = () => { setIsDragging(false); lastTouchDistance.current = null; };

    return (
        <div className="fixed inset-0 z-[200] bg-black/95 animate-fade-in overflow-hidden cursor-move" onWheel={handleWheel} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            <div className="w-full h-full flex items-center justify-center transition-transform duration-75 ease-out will-change-transform" style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})` }}>
                <img src={imageUrl} className="max-w-none md:max-w-full md:max-h-full object-contain select-none pointer-events-none shadow-2xl" alt="Zoom View" />
            </div>
            <div className="fixed top-6 right-6 z-[210] flex flex-col gap-3">
                 <button onClick={onClose} className="p-3 bg-white/10 text-white rounded-full hover:bg-red-500/80 backdrop-blur-md transition-all"><X size={24} /></button>
                 <div className="h-px bg-white/20 my-1"></div>
                 <button onClick={() => setScale(s => Math.min(s + 0.5, 5))} className="p-3 bg-white/10 text-white rounded-full hover:bg-white/30 backdrop-blur-md transition-all"><ZoomIn size={24} /></button>
                 <button onClick={() => setScale(s => Math.max(0.5, s - 0.5))} className="p-3 bg-white/10 text-white rounded-full hover:bg-white/30 backdrop-blur-md transition-all"><ZoomOut size={24} /></button>
                 <button onClick={() => { setScale(1); setPosition({ x: 0, y: 0 }); }} className="p-3 bg-white/10 text-white rounded-full hover:bg-white/30 backdrop-blur-md transition-all" title="Resetar"><RotateCcw size={20} /></button>
            </div>
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-black/50 px-4 py-2 rounded-full text-white text-xs font-mono backdrop-blur-md pointer-events-none">{Math.round(scale * 100)}%</div>
        </div>
    );
};

export const AtrioModal: React.FC<AtrioModalProps> = ({ item, onClose, onShowToast }) => {
  ...
  useEffect(() => {
      ...
      if (!isAuthor) {
          const loadStatus = async () => {
              ...
              const friendStatus = await connectionService.getFriendshipStatus(item.authorId);
  ...
  useEffect(() => {
      if (showSaveOptions) {
          const loadLists = async () => {
              ...
              const containing = lists.filter(l => l.itemIds.includes(item.id)).map(l => l.id);
  ...
  const handleSaveMainClick = () => {
      ...
      if (isSaved && !showSaveOptions) {
          const savedLists = availableLists.filter(l => listsContainingItem.includes(l.id));
  ...
  const handleListCreated = async (name: string, description: string, tags: string[]) => {
  ...
  const handleFollowClick = async () => {
      ...
      if (isFollowing) {
          await transactionService.processUnfollowTransaction('current_user', item.authorId);
          setIsFollowing(false);
          onShowToast("Vibe Restituída");
      } else {
          const result = await transactionService.processFollowTransaction('current_user', item.authorId);
          if (result.success) {
              setIsFollowing(true);
  ...
  const handleChatOrAddClick = async () => {
  ...
  if (isZoomed) {
      return <ZoomViewer imageUrl={item.url} onClose={() => setIsZoomed(false)} />;
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0f172a] animate-fade-in">
      <div className="w-full h-full overflow-y-auto overflow-x-hidden relative scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
          ...
          <div className="sticky top-0 left-0 w-full h-[75vh] z-0">
              ...
              <button 
                  ...
                  title="Zoom Livre"
              >
                  <ZoomIn size={24} className="group-hover:rotate-12 transition-transform" />
              </button>
          </div>
              ...
              <div className="w-full min-h-[50vh] bg-slate-900/70 backdrop-blur-[40px] border-t border-white/10 rounded-t-[3rem] px-6 py-10 md:p-12 shadow-[0_-20px_60px_rgba(0,0,0,0.7)] transition-all">
                  <div className="max-w-3xl mx-auto">
                      <div className="flex flex-col items-center text-center mb-10">
                          <div className={`h-1.5 w-24 rounded-full mb-8 ${item.color} shadow-[0_0_15px_currentColor]`}></div>
                          <h2 className="text-4xl md:text-6xl font-bold text-white tracking-tight drop-shadow-lg mb-4">{item.title}</h2>
                          <div className="flex items-center gap-2 text-[#50c878] bg-[#50c878]/10 px-4 py-1.5 rounded-full border border-[#50c878]/20">
                              <Maximize2 size={12} />
                              <span className="uppercase tracking-widest text-[10px] font-bold">Leitura Imersiva</span>
                          </div>
                      </div>
                      
                      <div className="prose prose-invert prose-lg max-w-none text-slate-200 font-light mb-16">
                          {item.description.split('\n\n').map((paragraph, index) => (
                              <p key={index} className="leading-loose text-lg md:text-xl text-justify md:text-left mb-8 last:mb-0 opacity-90 hover:opacity-100 transition-opacity">
                                  {paragraph}
                              </p>
                          ))}
                      </div>
                        ...
                        <div className="flex items-center justify-between gap-6">
                            ...
                            >
                                <img 
                                    ...
                                    className="w-16 h-16 rounded-full border-2 border-[#50c878]/30 p-1 group-hover:border-[#50c878] transition-colors" 
                                    alt={item.authorName} 
                                />
                                <div className="text-left">
                                    <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-1">Criado por</p>
                                    <h3 className="text-xl font-bold text-white group-hover:text-[#50c878] transition-colors">{item.authorName || 'Artista Anônimo'}</h3>
                                </div>
                            </div>

                            {isAuthor ? (
                                <div className="flex items-center gap-2 bg-[#FFC300]/20 text-[#FFC300] px-4 py-2 rounded-full border border-[#FFC300]/50 shadow-lg">
                                    <User size={18} fill="currentColor" /> 
                                    <span className="font-bold text-sm tracking-wide">Você é o Criador</span>
                                </div>
                            ) : (
                                <div className="flex items-center gap-3">
                                    <button 
                                        ...
                                        className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all shadow-lg flex items-center gap-2 ${
                                            isFollowing 
                                            ? 'bg-transparent border border-[#50c878] text-[#50c878] hover:bg-[#50c878]/10' 
                                            : 'bg-[#50c878] hover:bg-[#50c878]/90 text-[#1e293b]'
                                        }`}
                                    >
                                        {isFollowLoading ? <Loader2 className="animate-spin" size={16} /> : (
                                            <>
                                                <Footprints size={18} fill={isFollowing ? "currentColor" : "none"} />
                                    ...
                                    <button 
                                        ...
                                        disabled={isFriendLoading || friendshipStatus === 'pending_sent' || friendshipStatus === 'pending_received'}
                                        className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all border flex items-center gap-2 ${
                                    ...
                                    >
                                        {isFriendLoading ? <Loader2 className="animate-spin" size={16} /> : (
                                            <>
                                                {friendshipStatus === 'accepted' && <MessageCircle size={18} />}
                                                {(friendshipStatus === 'pending_sent' || friendshipStatus === 'pending_received') && <CheckCircle size={18} />}
                                                {friendshipStatus === 'none' && <UserPlus size={18} />}

                                                {friendshipStatus === 'accepted' && 'Conversar'}
                                                {(friendshipStatus === 'pending_sent' || friendshipStatus === 'pending_received') && 'Conectado'}
      ...
      <div 
        className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-auto animate-fade-in-up"
      >
        <div className="flex items-center gap-1 p-2 bg-[#0f172a]/60 backdrop-blur-xl rounded-full border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)] ring-1 ring-white/5 relative">
          <button 
             ...
             className={`bubbly-button group relative flex items-center gap-3 px-8 py-4 rounded-full transition-all active:scale-95 cursor-pointer border 
                ${hasVibed 
                    ? 'bg-[#FFC300]/20 text-[#FFC300] border-[#FFC300]/40' 
          ...
          >
            ...
            />
            <span className={`font-bold text-base tracking-wide hidden md:inline ${hasVibed ? 'text-[#FFC300]' : ''}`}>
          ...
          <button className="p-4 rounded-full text-slate-300 hover:text-[#50c878] hover:bg-[#50c878]/10 transition-all active:scale-95 cursor-pointer" title="Compartilhar">
            <Send size={24} />
          </button>
        </div>
      </div>
...
export interface CreateAtrioModalProps { onClose: () => void; onCreate: (item: Omit<AtrioItem, 'id' | 'vibes' | 'authorId'>) => Promise<void>; }

export const CreateAtrioModal: React.FC<CreateAtrioModalProps> = ({ onClose, onCreate }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const finalDesc = tag ? `Essência: #${tag}\n\n${description}` : description;
        ...
        <div className="fixed inset-0 z-[1001] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-[#1e293b] w-full max-w-4xl h-[90vh] md:h-auto md:max-h-[85vh] rounded-3xl border border-slate-700 shadow-2xl flex flex-col md:flex-row overflow-hidden animate-scale-up relative">
                ...
                <div className="w-full md:w-1/2 bg-[#0f172a] relative flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-800 h-56 md:h-auto shrink-0 group">
                     {mediaPreview ? (
                         <>
                            <img src={mediaPreview} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-all duration-500" alt="Preview" />
                            <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-[2px]">
                                <button onClick={() => fileInputRef.current?.click()} className="p-3 bg-white/10 hover:bg-white/20 text-white rounded-full backdrop-blur-md border border-white/10 transition-transform hover:scale-110">
                                    <Camera size={24} />
                                </button>
                            </div>
                         </>
                         ...
                         >
                             ...
                             <p className="text-slate-500 text-xs">Alta resolução recomendada</p>
                         </div>
                     )}
                ...
                <div className="w-full md:w-1/2 flex flex-col bg-[#1e293b] flex-1 min-h-0">
                    ...
                    <div className="p-6 space-y-5 flex-1 overflow-y-auto custom-scrollbar">
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mb-1 block">Título da Obra</label>
                            <input 
                                value={title} 
                        ...
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mb-1 block">Tag / Essência</label>
                        ...
                        <div>
                            <label className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mb-1 block">Reflexão Profunda</label>
                            <textarea 
                                value={description} 
                    ...
                    <div className="p-6 border-t border-slate-700/50 bg-[#1e293b]">
                        ...
                        >
                            {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : (
                                <>
                                    Publicar no Átrio 
                                    <Zap size={20} className="text-[#FFC300] fill-[#FFC300] group-hover:scale-110 transition-transform" />
                                </>
                            )}
                        </button>
                    </div>
...
const AtrioLeveza: React.FC = () => {
    ...
    const handleCreateItem = async (newItemData: Omit<AtrioItem, 'id' | 'vibes' | 'authorId'>) => {
    ...
    return (
        <div className="min-h-screen pb-24 px-4 py-6 md:px-8">
                    ...
                    {items.map(item => (
                        ...
                        >
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                                ...
                                <p className="text-slate-300 text-xs truncate">by {item.authorName}</p>
                            </div>
                        </div>
                    ))}
                    {items.length === 0 && (
            ...
            {isCreateModalOpen && <CreateAtrioModal onClose={() => setIsCreateModalOpen(false)} onCreate={handleCreateItem} />}
