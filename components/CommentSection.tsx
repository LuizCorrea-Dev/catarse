...
import { Send, AlertCircle, Clock, Zap, Trash2, Loader2, AlertTriangle } from 'lucide-react';
...
const CommentSection: React.FC<CommentSectionProps> = ({ postId, authorId, onVibeTransfer, canDelete = false }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (charCount < minChars) {
      setError(`Faltam ${remaining} caracteres para atingir a profundidade mínima.`);
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        const newCommentObj: Comment = {
          id: Date.now().toString(), // Em produção, ID viria do DB
          postId: postId,
          userId: 'current_user',
          authorName: 'Você',
  ...
  const confirmDelete = async () => {
      ...
      const updatedBalance = await transactionService.processCommentRefund(commentToDelete.id, commentatorId);
  ...
  const handleLike = async (e: React.MouseEvent, comment: Comment) => {
      ...
      try {
          // CORREÇÃO: Passando 'comment' como 4º argumento para usar comment_id no banco
          const result = await transactionService.processLikeTransaction(commentId, 'current_user', commentAuthorId, 'comment');
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-700/50" onClick={(e) => e.stopPropagation()}>
      <h3 className="text-sm font-bold text-slate-400 mb-4 flex items-center gap-2">
        <span className="bg-slate-700/50 px-2 py-0.5 rounded text-xs">Transação: 1 VIBE</span>
      ...
      <div className="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
        {isLoading ? (
            ...
            comments.map(comment => {
                ...
                const hasLikedLocal = transactionService.hasLiked(comment.id, 'current_user');
                const isLiking = likingComments.has(comment.id);
                const timeAgo = formatTime(comment.createdAt);

                return (
                    <div key={comment.id} className="flex gap-3 group">
                        <img src={comment.authorAvatar} alt={comment.authorName} className="w-8 h-8 rounded-full object-cover mt-1 opacity-80" />
                        ...
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-[#50c878] text-xs font-bold">{comment.authorName}</span>
                            <div className="flex items-center gap-3">
                                ...
                                <button 
                                    ...
                                    title={isMyComment ? "Você não pode curtir seu próprio comentário" : "Enviar Vibe (+1)"}
                                >
                                    <Zap size={10} fill={hasLikedLocal ? "currentColor" : "none"} />
                                    {comment.vibes}
                                </button>
      ...
      <form onSubmit={handleSubmit} className="relative bg-slate-900/60 p-4 rounded-xl border border-slate-700/50 focus-within:border-[#50c878]/50 transition-colors">
        <textarea
          value={newComment}
        ...
        <div className="flex justify-between items-center">
          <span className={`text-xs ${charCount >= minChars ? 'text-[#50c878]' : 'text-slate-500'}`}>
        ...
        >
          ...
          >
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-bold text-white mb-2">Apagar comentário?</h3>
              ...
              <div className="text-slate-400 text-xs mb-6 leading-relaxed space-y-2">
                 ...
                 <p>
                    A <strong>1 VIBE</strong> que você gastou para comentar <strong>NÃO</strong> será devolvida.
                 </p>
                 <p className="text-slate-500">
                    Porém, qualquer VIBE que este comentário recebeu de outros usuários será estornada para eles.
                 </p>
              </div>
              ...
              <div className="flex gap-3 w-full">
                ...
                >
                  {isDeleting ? <Loader2 size={14} className="animate-spin" /> : 'Apagar'}
                </button>
              </div>
            </div>
          </div>
