    ...
    return (
      <div className="w-full max-w-md bg-[#1e293b]/90 backdrop-blur-md p-8 rounded-3xl border border-slate-700 shadow-2xl relative overflow-hidden text-center">
        ...
        <p className="text-slate-400 text-sm mb-8 leading-relaxed">
          ...
          Por favor, verifique sua caixa de entrada (e spam) para ativar sua
          conta e começar sua jornada.
        </p>
        <button
    ...
    <div className="w-full max-w-md bg-[#1e293b]/90 backdrop-blur-md p-8 rounded-3xl border border-slate-700 shadow-2xl relative overflow-hidden">
      ...
      <form onSubmit={handleAuthSubmit} className="space-y-4 relative z-10">
        {!isLogin && (
          <>
            <div className="relative group">
              <Smile
                className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#50c878] transition-colors"
                size={20}
              />
              <input
                type="text"
            ...
            {usernameError && (
              <p className="text-xs text-red-400 pl-2 -mt-2">{usernameError}</p>
            )}
          </>
        )}
        ...
        <button
          type="submit"
          disabled={
            isLoading || (!isLogin && (!usernameAvailable || !fullName))
          }
